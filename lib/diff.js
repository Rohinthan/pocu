 ...
 */
function confirm(question, rl = null) {
  if (process.env.POCU_YES === '1' || process.argv.includes('-y') || process.argv.includes('--yes')) {
    console.log(`${question} y (auto)`);
    return Promise.resolve(true);
  }
  if (rl && typeof rl.question === 'function' && !rl.closed) {
  ...
  const localRl = readline.createInterface({ input: process.stdin, output: process.stdout });
  return new Promise((resolve) => {
    try {
      localRl.question(`${question} `, (answer) => {
        localRl.close();
