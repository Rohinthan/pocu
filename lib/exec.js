...
async function runC(filePath) {
  ...
  const run = await execPromise(tmpBinary, []);
  try {
...
async function runPython(filePath) {
  ...
  const run = await execPromise(bin, [filePath]);
  return { stage: 'run', ...run };
}

async function runNode(filePath) {
  const run = await execPromise(process.execPath, [filePath]);
  return { stage: 'run', ...run };
}
 ...
 */
async function executeFile(filePath, content) {
  ...
  const result = await lang.run(filePath);
